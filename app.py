...
.history-row { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:6px; padding:0.6rem 1rem; margin-bottom:0.4rem; display:flex; justify-content:space-between; align-items:center; font-size:0.85rem; }
</style>
...
# ── Helper: construir array y predecir ────────────────────────────────────────
...
with tab2:
    ...
    st.markdown("Visualiza qué variables tiene más peso en el modelo y cómo los valores actuales se comparan con el umbral de pit stop.")
 
    # Importancia del modelo (feature importance si está disponible)
    ...
    if tiene_importancia or tiene_coef:
        ...
        ax.yaxis.set_tick_params(color='#333')
    ...
    # Radar / perfil del dato actual vs promedios típicos
    ...
    dato_norm = [
        ...
        (Cumul_Deg + 80) / 90,
        RaceProgress,
        round(TyreLife / 58.0, 4),
        (Position_Change + 5) / 10
    ]
...
# ─────────────────────────────────────────────────────────────────────────────
# TAB 3 · SIMULADOR VUELTA A VUELTA
# ─────────────────────────────────────────────────────────────────────────────
with tab3:
    ...
    if st.button("▶  SIMULAR CARRERA", key="btn_sim"):
        ...
        for v in vueltas:
            ...
            p, prob  = predecir(sim_compound, 1, v, sim_position,
                                lap_t, delta, cumul_d, progress, 0)
            probs.append(prob)
            decisiones.append(p)
 
        # Identificar vueltas de pit stop recomendadas
        ...
        # Marcar vueltas de pit stop recomendadas
        if pit_vueltas:
...
with tab4:
    ...
    # Métricas conocidas del modelo
    col_m1, col_m2, col_m3, col_m4 = st.columns(4)
    metricas = [
        ("71%",  "Accuracy",   "#ffd700"),
        ("0.92", "ROC-AUC",    "#ff1801"),
        ("~0.68","F1-Score",   "#00e676"),
        ("~0.74","Recall",     "#0067ff"),
    ]
    ...
    with col_cm:
        ...
        # Matriz representativa del modelo
        cm = np.array([[8420, 980],
                       [1230, 3870]])
        ...
        im = ax5.imshow(cm, cmap='Reds', alpha=0.7)
        for i in range(2):
            for j in range(2):
...
# ─────────────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div style="font-family:Orbitron,monospace; font-size:0.7rem; color:#ff1801; letter-spacing:0.2em; text-transform:uppercase; border-bottom:1px solid #ff1801; padding-bottom:0.4rem; margin-bottom:1rem">📋 Historial de Sesión</div>', unsafe_allow_html=True)
    ...
    if st.session_state.historial:
        ...
        for i, h in enumerate(reversed(st.session_state.historial), 1):
            color_d = '#ff1801' if 'PIT' in h['Decisión'] else '#00e676'
            st.markdown(f"""
            <div style="background:rgba(255,255,255,0.03); border-left:3px solid {color_d}; border-radius:4px; padding:0.5rem 0.7rem; margin-bottom:0.5rem; font-size:0.78rem;">
              <div style="color:{color_d}; font-weight:700">{h['Decisión']}</div>
              <div style="color:#888; margin-top:0.2rem">{h['Compuesto']} · {h['TyreLife']} vueltas · P{h['Posición']}</div>
              <div style="color:#666">{h['LapTime']} · {h['Progreso']} carrera · {h['Prob']}</div>
            </div>""", unsafe_allow_html=True)
    else:
